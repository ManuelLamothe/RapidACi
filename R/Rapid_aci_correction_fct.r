...
#' @param leafArea_df A dataframe containing at least a sample_ID column and leafArea_mm2
#'   column
...
Rapid_aci_correction <- function(list_files,
  ...
  # If leafArea are provided, empty chamber files needs to get the same 'real' value 
  # (Const_S) than the measures files before getting the correction coefficients
  if (is.null(leafArea_df)) {list_files$leafArea_mm2 <- NA
  } else {list_files <- left_join(list_files, select(leafArea_df, sample_ID, "leafArea_mm2"))}
