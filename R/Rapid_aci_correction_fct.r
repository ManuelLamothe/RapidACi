...
Rapid_aci_correction <- function(list_files,
  ...
  # Retrieve Rd from Dark files (with leaf area adjustment if available)
  # first draft: multiple dark files ok, but not duplicated sample_ID names
  dark_files <- dplyr::filter(list_files, chamber == "DARK")
  
  if(dim(dark_files)[1] > 0) {
    merged_dark <- vector("list", length = nrow(dark_files))
    LA <- select(list_files, sample_ID, leafArea_mm2) %>% drop_na()
    for(i in 1:nrow(dark_files)) {
      merged_dark[[i]] <- left_join(list_files, 
                                    correct_dark(dark_files$path[i], leafArea = LA,
